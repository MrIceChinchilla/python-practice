print('Итак, вы секретный агент, цель которого спасти заложника из рук бандитов. С собой у вас только:')
print('Нож, пистолет с патронами и граната на крайний случай. Удачи!\n\n')
print('Вы стоите прямо перед зданием!')
print('1) Осмотреть здание\n2) Войти внутрь\n3) Бросить гранату в дверь')
answer = int(input())
if answer == 1:
    print('Итак, вы осмотрели здание, оно 3-ёх этажное, спереди балкон. На первом этаже есть 2 окна, но одно из них заколочено досками, рядом с ними лежит молоток, второе окно пустое.\n')
    print('1) Войти в пустое окно\n2) Вернуться ко входу\n3) Разбить доски молотком\n4) Подойти к пустому окну и взять с собой молоток\n5) Бросить гранату в пустое окно')
    answer1 = int(input())
    if answer1 == 1:
        print('Не успел я подойти к окну, как тут-же меня подстрелили.\n---Наступила мучительная смерть от экспансивных боеприпасов в живот---')
    elif answer1 == 2:
        print('Чёрт! меня заметил снайп...\n---Наступила смерть, хоть и безболезненная, 1 выстрел в голову и всё!---')
    elif answer1 == 3:
        print('Кажется я изрядно пошумел... О нет пришли охранники! Что делать?\n1) Перестрелять их всех!\n2) Спрятаться под заколоченым окном\n3) Закинуть им гранату\n4) Притвориться мёртвым')
        answer2 = int(input())
    elif answer1 == 4:
        print('Я так и думал! Там стоял один из бандитов.\n1) Застрелить гада\n2) Отвлечь охранника броском молотка\n3) Притвориться заложником и выбросить оружия\n4) Подождать')
        answer3 = int(input())
        if answer3 == 1:
            print('Бах! И нет бандита, О нет! из-за угла вбежал другой бандит! Не успел я ничего придумать ничего, как сразу словаил лбом "маслину"\n---Быстрая смерть, что тут сказать---')
        elif answer3 == 2:
            print('Ух-ты, сработало, этот тупица пошёл смотреть что и куда упало. Прям как в фильмах!Ах да мне же нельзя ломать 4-тую стену. Что делать теперь?')
            print('\n1) Войти в пустое окно\n2) Метнуть в охранника нож\n3) Застрелить бандюгана')
            answer4 = int(input())
            if answer4 == 3:
                print('Бах! И нет бандита, О нет! из-за двери вбежал другой бандит! Не успел я ничего придумать ничего, как сразу словил лбом "маслину"\n---Быстрая смерть, что тут сказать---')
            elif answer4 == 1:
                print('Temp')
            elif answer4 == 2:
                print('Итак я бросил в охранника нож. В яблочко! Бандит упал замертво, кажется там тихо.\n1) Влезть в окно\n2) Подождать')
                answer5 = int(input())
                if answer5 == 1:
                    print('Итак я вошёл в эту тёмную комнату, вокруг очень темно, но я вижу кучу мусора на деревянном письменном столе, тело убитого мной охранника и пару дверей.\n1) Осмотреть мусор на столе\n2) Осмотреть комнату\n3) Осмотреть тело\n4) Подождать')
                    answer6 = int(input())
                    if answer6 == 1:
                        print(u'В мусоре на столе я не нашёл ничего необычного. Я подобрал: Шнур от утюга, патроны для дробовика, стеклянную бутылку и записку.\nВ записке написано:\n\nДорогая, мне очень жаль, что всё так вышло\nДжонни сказал что заплатит сразу\nПосле поимки заложников, но похоже,\nЧто он обманул меня. Здание\n Уже оцепили легавые, надеюсь,\nЧто мы ещё увидимся.\n\n Какая трогательная история! Но мне пора идти дальше.')
                        print('Итак я вошёл в эту тёмную комнату, вокруг очень темно, но я вижу тело убитого мной охранника и пару дверей.\n1) Осмотреть комнату\n2) Осмотреть тело\n3) Подождать') 
                        answer7 = int(input())
                        if answer7 == 1:
                           print('Это небольшая комната с окном и двумя дверьми. На полу лежит тело бандита, которого я ликвидировал броском ножа в шею, кстати заберу-ка я нож. В комнате есть письменный стол, шкаф и стул. А так-же пару дверей, Я заглянул в дверную щель первой двери, из неё изходил свет. Видимо там кто-то есть. Похоже на туалет. У щели другой двери виден тёмный корридор. Из корридора слышны разговоры бандитов. Блин где-же заложники? Ладно надо осмотреть тело бедного разбойника\nХмм... Итак, я разжился: ') 
        elif answer3 == 3:
            print('Temp')
elif answer == 2:
    print('Как только я вошёл внутрь меня просто расстреляли!')
    print('---Наступила мучительная и негуманная казнь---')   
elif answer == 3:
    print('Я быстро открыл дверь, закинул туда гранату и закрыл её. Неужели успех?\nВдруг мне позвонило начальство и сообщило, что заложники уже мертвы. Ведь как сказали бандиты,"Пусть только шмальнут! Я тут всех заложников положу!"')
    print('---Миссия провалена---')
else:
    print('Неверный вариант ответа')
